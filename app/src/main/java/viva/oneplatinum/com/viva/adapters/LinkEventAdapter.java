...
/**
 * Created by D-MAX on 5/17/2016.
 */
public class LinkEventAdapter extends ArrayAdapter<Event> {
    ...
    public LinkEventAdapter(Context context, int textViewResourceId, ArrayList<Event> list,String token,Boolean isFromMyEvent) {
...
//        if(mNotification.eventImages.size()>0) {
//            aq1.id(viewHolder.event_image).image(mNotification.eventImages.get(0), true, true, 400, 0,
...
//                    });
//        }
        aq.id(viewHolder.event_image).image(mEvent.mainImage, true, true, 400, 0,
                new BitmapAjaxCallback() {
                    protected void callback(String url, ImageView iv,
                                            Bitmap bm, AjaxStatus status) {
                        if (bm != null) {
