package viva.oneplatinum.com.viva.dynamicgrid;
...
public class DynamicGridView extends GridView {
	...
	public void startEditMode(int position) {
		...
		if (position != -1) {
			startDragAtPosition(position, 0, 0);
		}
		mIsEditMode = true;
	...
	public void init(Context context) {
		...
		imageDrag.setImageDrawable(mHoverCell);
		pop.setContentView(imageDrag);
		pop.getBackground().mutate()
				.setColorFilter(Color.parseColor("#63ade6"), Mode.SCREEN);
		Rect r = new Rect();
		this.getLocalVisibleRect(r);
		Log.d("viewBoundsForGridView", top + " " + right + " " + bottom + " "
				+ left);
		Log.d("viewRectforGrid", r.left + " " + r.top);
	}
	 ...
	 */
	private BitmapDrawable getAndAddHoverView(View v, int x, int y) {
		...
		// Also add a popup view
		Bitmap d = getBitmapFromView(v);
		BitmapDrawable db = new BitmapDrawable(getResources(), b);
		drawable.setAlpha(0);
		imageDrag.setImageDrawable(db);
		pop.setHeight(h);
		pop.setWidth(w);
		pop.setFocusable(false);
		pop.showAtLocation(this, Gravity.NO_GRAVITY, location[0], location[1]);
		...
		switch (event.getAction() & MotionEvent.ACTION_MASK) {
			case MotionEvent.ACTION_DOWN:
				...
				if (mIsEditMode && isEnabled()) {
					...
					startDragAtPosition(position, (int) event.getRawX(),
							(int) event.getRawY());
				} else if (!isEnabled()) {
					return false;
				}
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (mCellIsMobile) {
					...
					if (mDragListener != null) {
						mDragListener.onHoverStarted(mHoverCell.getBounds(),
								mLastEventX, mLastEventY, imageDrag);
					}
					// pop.update(mHoverCellOriginalBounds.left + deltaX
					// + mTotalOffsetX, mHoverCellOriginalBounds.top + deltaY
					// + mTotalOffsetY+300, -1, -1);
					pop.update(
							(int) event.getRawX()
									- ((mHoverCellCurrentBounds.right - mHoverCellCurrentBounds.left) / 2),
							(int) event.getRawY()
									- ((mHoverCellCurrentBounds.bottom - mHoverCellCurrentBounds.top) / 2),
							-1, -1);
					invalidate();
					handleCellSwitch();
					mIsMobileScrolling = false;
			...
			case MotionEvent.ACTION_UP:
				...
				if (mHoverCell != null) {
					if (mDropListener != null) {
						Rect r = mHoverCell.getBounds();
						int pos = getPositionForID(mMobileItemId);
						mDropListener
								.onActionDrop(r, mLastEventX, mLastEventY, pos);
					}
				}
				break;
			...
			case MotionEvent.ACTION_CANCEL:
				...
				if (mHoverCell != null) {
					if (mDropListener != null) {
						mDropListener.onActionDrop(null, 0, 0, -1);
					}
				}
				break;
	...
	private void startDragAtPosition(int position, int x, int y) {
		mTotalOffsetY = 0;
		mTotalOffsetX = 0;
		DragPosition = position;
		int itemNum = position - getFirstVisiblePosition();
		View selectedView = getChildAt(itemNum);
		viewDrag = selectedView;
		if (selectedView != null) {
			...
			mHoverCell = getAndAddHoverView(selectedView, x, y);
	...
	public interface OnDropListener {
		void onActionDrop(Rect r, int x, int y, int pos);
	}
